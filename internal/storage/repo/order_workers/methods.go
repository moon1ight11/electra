...
func (r *OrderWorkerRepo) GetByOrder(ctx context.Context, orderID uuid.UUID) ([]domain.OrderWorker, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to get order workers: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&ow.ID,
			...
			&ow.MaterialsUsed,
			&ow.Notes); err != nil {
			return nil, fmt.Errorf("failed to scan order worker: %w", err)
		}
		ows = append(ows, ow)
	}
...
func (r *OrderWorkerRepo) GetByOrderAndWorker(ctx context.Context, orderID, workerID uuid.UUID) (*domain.OrderWorker, error) {
	...
	err := r.db.DB.QueryRowContext(ctx, query, orderID, workerID).Scan(
		&ow.ID,
		&ow.OrderID,
		&ow.WorkerID,
		&ow.TimeSpent,
	...
	if err != nil {
		...
		return nil, fmt.Errorf("failed to get order worker: %w", err)
	}
	return ow, nil
}
...
func (r *OrderWorkerRepo) Update(ctx context.Context, ow *domain.OrderWorker) error {
	...
	if err != nil {
		return fmt.Errorf("failed to update order worker: %w", err)
	}
	return nil
}
...
func (r *OrderWorkerRepo) Remove(ctx context.Context, orderID, workerID uuid.UUID) error {
	query := `DELETE FROM order_workers WHERE order_id = $1 AND worker_id = $2`

	_, err := r.db.DB.ExecContext(ctx, query, orderID, workerID)
	if err != nil {
		return fmt.Errorf("failed to remove order worker: %w", err)
	}
	return nil
}